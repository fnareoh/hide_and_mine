...
if metric == "ghosts":
    plot = [data["ghosts"][i] for i in range(4)]
else:
    plot = [data["distortion"][i] for i in range(4)]
...
rects1 = ax.bar(x - width / 2, plot[0], width / 2, label="TPM")
rects2 = ax.bar(x, plot[1], width / 2, label="ILP")
rects3 = ax.bar(x + width / 2, plot[3], width / 2, label="HEU")  # h2
